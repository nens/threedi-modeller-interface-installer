...
# Modified by Marco Duiker for Nelen en Schuurmans 3Di Modeller Interface
...
unless(@ARGV) {
	print "Defaulting to qgis-ltr-full package...\n" if $verbose;
...
# this one is superseded by the postinstall-nl.bat as used in the 3Di-additions
# but rather we should generate our changes into this and find a way to skip the qgis.bat
# from running

open F, ">../Installer-Files/postinstall.bat";

my $r = ">>postinstall-3di.log 2>&1\r\n";

print F "\@echo off\r\n";
print F "if exist postinstall-3di.log del postinstall-3di.log\r\n";
...
print F "ren postinstall-3di.bat postinstall-3di.bat.done$r";
...
# copy the 3Di modeller interface additions
my $obj;
my @excludes = ('.*');
...
$obj->exec( src => "$cwd_dir/3Di-additions/", dest => "$cwd_dir/QGIS" )
    or warn "rsync failed\n";

chdir "QGIS/ms-windows";
...
if( @lic ) {
    ...
    print O "1. 3Di Modeller Interface\n";
	print O "2. QGIS\n";
	$i = 2;
	for my $l ( @desc ) {
		print O ++$i . ". $l\n";
	...
	$i = 0;
    print O "\n\n----------\n\n" . ++$i . ". License of '3Di Modeller Interface'\n\n";
...
$args .= " -DPROFILE_FOLDER=profiles";
$args .= " -DLICENSE_FILE='$license'";
$args .= " -DARCH='$arch'";
$args .= " 3Di-Installer.nsi";
