...
*/
extern DLLEXPORT int lwtype_get_collectiontype(uint8_t type);
 ...
 */
extern DLLEXPORT POINT4D getPoint4d(const POINTARRAY *pa, int n);
 ...
 */
extern DLLEXPORT int getPoint4d_p(const POINTARRAY *pa, int n, POINT4D *point);
 ...
 * NOTE: point is a real POINT3D *not* a pointer
 */
extern DLLEXPORT POINT3DZ getPoint3dz(const POINTARRAY *pa, int n);
extern DLLEXPORT POINT3DM getPoint3dm(const POINTARRAY *pa, int n);
 ...
 */
extern DLLEXPORT int getPoint3dz_p(const POINTARRAY *pa, int n, POINT3DZ *point);
extern DLLEXPORT int getPoint3dm_p(const POINTARRAY *pa, int n, POINT3DM *point);
 ...
 * NOTE: point is a real POINT3D *not* a pointer
 */
extern DLLEXPORT POINT2D getPoint2d(const POINTARRAY *pa, int n);
 ...
 */
extern DLLEXPORT int getPoint2d_p(const POINTARRAY *pa, int n, POINT2D *point);
...
*/
extern DLLEXPORT const POINT2D* getPoint2d_cp(const POINTARRAY *pa, int n);
...
* values, only read them.
*/
extern DLLEXPORT const POINT3DZ* getPoint3dz_cp(const POINTARRAY *pa, int n);
...
* values, only read them.
*/
extern DLLEXPORT const POINT4D* getPoint4d_cp(const POINTARRAY *pa, int n);
 ...
 */
extern DLLEXPORT void ptarray_set_point4d(POINTARRAY *pa, int n, const POINT4D *p4d);
 ...
 */
extern DLLEXPORT uint8_t *getPoint_internal(const POINTARRAY *pa, int n);

/*
 * size of point represeneted in the POINTARRAY
 * 16 for 2d, 24 for 3d, 32 for 4d
 */
extern DLLEXPORT int ptarray_point_size(const POINTARRAY *pa);
...
*/
extern DLLEXPORT int ptarray_insert_point(POINTARRAY *pa, const POINT4D *p, int where);

/**
* Remove a point from an existing #POINTARRAY. Zero
* is the index of the start of the array.
*/
extern DLLEXPORT int ptarray_remove_point(POINTARRAY *pa, int where);
...
extern DLLEXPORT void ptarray_longitude_shift(POINTARRAY *pa);
extern DLLEXPORT int ptarray_isccw(const POINTARRAY *pa);
extern DLLEXPORT void ptarray_reverse(POINTARRAY *pa);
 ...
 */
extern DLLEXPORT int lwline_add_lwpoint(LWLINE *line, LWPOINT *point, int where);
...
extern DLLEXPORT double next_double_down(float d);
extern DLLEXPORT double next_double_up(float d);
...
extern DLLEXPORT LWPOINT* lwline_get_lwpoint(const LWLINE *line, int where);
extern DLLEXPORT LWPOINT* lwcircstring_get_lwpoint(const LWCIRCSTRING *circ, int where);
...
extern DLLEXPORT LWPOINT* lwcompound_get_lwpoint(const LWCOMPOUND *lwcmp, int where);


extern DLLEXPORT double ptarray_length_2d(const POINTARRAY *pts);
extern DLLEXPORT double ptarray_length(const POINTARRAY *pts);
extern DLLEXPORT double ptarray_arc_length_2d(const POINTARRAY *pts);
...
extern DLLEXPORT void lwgeom_reverse(LWGEOM *lwgeom);
extern DLLEXPORT void lwline_reverse(LWLINE *line);
extern DLLEXPORT void lwpoly_reverse(LWPOLY *poly);
extern DLLEXPORT void lwtriangle_reverse(LWTRIANGLE *triangle);
...
extern DLLEXPORT void lwpoly_force_clockwise(LWPOLY *poly);
extern DLLEXPORT void lwtriangle_force_clockwise(LWTRIANGLE *triangle);


extern DLLEXPORT void interpolate_point4d(POINT4D *A, POINT4D *B, POINT4D *I, double F);

void DLLEXPORT lwgeom_longitude_shift(LWGEOM *lwgeom);
...
*/
extern DLLEXPORT int lwgeom_count_vertices(const LWGEOM *geom);
...
*/
extern DLLEXPORT int lwgeom_count_rings(const LWGEOM *geom);
...
*/
extern DLLEXPORT int lwgeom_dimensionality(LWGEOM *geom);

/* Is lwgeom1 geometrically equal to lwgeom2 ? */
DLLEXPORT char lwgeom_same(const LWGEOM *lwgeom1, const LWGEOM *lwgeom2);
DLLEXPORT char ptarray_same(const POINTARRAY *pa1, const POINTARRAY *pa2);
DLLEXPORT char lwpoint_same(const LWPOINT *p1, const LWPOINT *p2);
DLLEXPORT char lwline_same(const LWLINE *p1, const LWLINE *p2);
DLLEXPORT char lwpoly_same(const LWPOLY *p1, const LWPOLY *p2);
DLLEXPORT char lwtriangle_same(const LWTRIANGLE *p1, const LWTRIANGLE *p2);
DLLEXPORT char lwcollection_same(const LWCOLLECTION *p1, const LWCOLLECTION *p2);
DLLEXPORT char lwcircstring_same(const LWCIRCSTRING *p1, const LWCIRCSTRING *p2);
...
DLLEXPORT LWPOINT *lwpoint_clone(const LWPOINT *lwgeom);
DLLEXPORT POINTARRAY *ptarray_clone_deep(const POINTARRAY *ptarray);
 ...
 */
extern DLLEXPORT LWGEOM *lwgeom_segmentize2d(LWGEOM *line, double dist);
extern DLLEXPORT POINTARRAY *ptarray_segmentize2d(const POINTARRAY *ipa, double dist);
extern DLLEXPORT LWLINE *lwline_segmentize2d(LWLINE *line, double dist);
extern DLLEXPORT LWPOLY *lwpoly_segmentize2d(LWPOLY *line, double dist);
extern DLLEXPORT LWCOLLECTION *lwcollection_segmentize2d(LWCOLLECTION *coll, double dist);

/*
 * Point density functions
 */
extern DLLEXPORT LWMPOINT *lwpoly_to_points(const LWPOLY *poly, int npoints);
extern DLLEXPORT LWMPOINT *lwmpoly_to_points(const LWMPOLY *mpoly, int npoints);
extern DLLEXPORT LWMPOINT *lwgeom_to_points(const LWGEOM *lwgeom, int npoints);
...
extern DLLEXPORT char lwtriangle_is_repeated_points(LWTRIANGLE *triangle);
...
*/
extern DLLEXPORT LWGEOM* lwgeom_flip_coordinates(LWGEOM *in);
...
*/
extern DLLEXPORT int getPoint2d_p_ro(const POINTARRAY *pa, int n, POINT2D **point);
...
extern DLLEXPORT char*   hexbytes_from_bytes(uint8_t *bytes, size_t size);
 ...
 */
extern DLLEXPORT LWGEOM* lwgeom_from_twkb(uint8_t *twkb, size_t twkb_size, char check);
...
int DLLEXPORT lwgeom_has_arc(const LWGEOM *geom);
LWGEOM DLLEXPORT *lwgeom_stroke(const LWGEOM *geom, uint32_t perQuad);
LWGEOM DLLEXPORT *lwgeom_unstroke(const LWGEOM *geom);
...
DLLEXPORT LWCOLLECTION *lwgeom_subdivide(const LWGEOM *geom, int maxvertices);
 ...
 * Requires GEOS-3.2.0+
 */
DLLEXPORT LWGEOM* lwgeom_offsetcurve(const LWLINE *lwline, double size, int quadsegs, int joinStyle, double mitreLimit);
...
DLLEXPORT int ptarray_transform(POINTARRAY *geom, projPJ inpj, projPJ outpj) ;
DLLEXPORT int point4d_transform(POINT4D *pt, projPJ srcpj, projPJ dstpj) ;
 ...
 */
DLLEXPORT LWGEOM* lwgeom_delaunay_triangulation(const LWGEOM *geom, double tolerance, int edgeOnly);
...
*/
DLLEXPORT int * lwgeom_cluster_2d_kmeans(const LWGEOM **geoms, int ngeoms, int k);
